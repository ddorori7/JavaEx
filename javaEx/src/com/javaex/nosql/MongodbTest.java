...
//		Bson bsonFilter = Filters.eq("species", "인간");
	...
	private static void testFindAll() {
		...
		// 루프
		while (cursor.hasNext()) { // 뒤에 남은 레코드가 있으면
			// 커서 뒤로 이동
			Document doc = cursor.next();
	...
	private static void testInsertMany() {
		...
		// 문서 생성 -> list 추가
		Document doc = new Document("name", "고길동").append("species", "인간").append("gender", "남성");
		docs.add(doc);
		doc = new Document("name", "도우너").append("species", "외계인").append("gender", "남성");
		docs.add(doc);
		doc = new Document("name", "또치").append("species", "조류").append("gender", "여성");
		docs.add(doc);
		doc = new Document("name", "영희").append("species", "인간").append("gender", "여성");
		docs.add(doc);
	...
	private static void testInsert() {
		...
		Document doc = new Document("name", "둘리").append("gender", "남성").append("species", "공룡");
