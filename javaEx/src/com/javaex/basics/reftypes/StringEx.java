...
//		stringType();
	...
	private static void stringFormatEx() {
		...
		// 현재 이자율은 ___% 입니다.
		fmt = "현재 이자율은 %f%% 입니다.%n";
		System.out.printf(fmt, rate); // 현재 이자율은 1.234000% 입니다.
		fmt = "현재 이자율은 %.2f%% 입니다.%n";
		System.out.printf(fmt, rate); // 소수점 2자리까지 표기 > 현재 이자율은 1.23% 입니다.
	}
	...
	private static void stringType() {
		...
		// 참조 타입에서 == 은 참조 주소가 같은지(같은 객체인지) 판별하는 연산자
		System.out.println("str == str2 ? " + (str == str2)); //주소가 같다
		System.out.println("str == str3 ? " + (str == str3)); //참조하는 주소가 다르다
